...
def process_user_choice(choice, person):
    if choice == '1':
        ...
        try:
            count = float(input('Сумма к снятию:')) #Проверка корректности ввода
        except ValueError:
...
def start():
    ...
    try:
        card_number = int(card_number)  #Проверка корректности ввода
    except ValueError:
        print('Неверный формат номера карты')
    try:
        pin_code = int(pin_code)  #Проверка корректности ввода
    except ValueError:
        print('Неверный формат пин кода')
